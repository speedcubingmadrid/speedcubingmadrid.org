class SolveTime
  ...
  def initialize(event_id, format, wca_value)
    @event_id = event_id
    @format = format
  ...
  def wca_value=(wca_value)
    ...
    if @event_id == "333fm"
      # The average format for 333fm is stored the same way as for other event_ids, multiplied by 100.
      # Otherwise, wca_value is simply the number of moves.
      @move_count = @format == "average" ? (wca_value / 100.0) : wca_value
    elsif @event_id == "333mbf"
      mb_value = wca_value
  ...
  def time_centiseconds=(time_centiseconds)
    raise "tiempo fuera de rango" unless 0 <= time_centiseconds && time_centiseconds <= 99_999 * 100
  ...
  def solved=(solved)
    raise "resueltos fuera de rango" unless (0...100).cover?(solved)
    @solved = solved
    recompute_wca_value
  end

  def attempted=(attempted)
    raise "intentados fuera de rango" unless (0...100).cover?(attempted)
    @attempted = attempted
    recompute_wca_value
  end
  ...
  def clock_format
    ...
    if @event_id == "333fm"
      format_str = (@format == "average" ? "%.2f" : "%.0f")
  ...
  private def units
    if incomplete?
      ""
    elsif (@event_id != "333fm" && @event_id != "333mbf")
      time_minutes >= 1 ? "" : " segundos"
    elsif @event_id == "333fm"
      " movimientos"
    elsif @event_id == "333mbf"
      ""
    else
      raise "Tipo de event_id no reconocido #{@event_id}"
    end
  end
  ...
  def multiblind_time_limit
    return unless @event_id.id == "333mbf" && complete?
    ...
    if time_seconds > (time_limit_seconds + 30)
      errors.add(:base, "debería ser menor o igual que #{time_limit_minutes} minutos")
    end
  end
  ...
  def times_over_10_minutes_must_be_rounded
    ...
    if ((@event_id != "333fm" && @event_id != "333mbf") || @event_id == "333mbf") && time_minutes > 10 && time_centiseconds % 100 > 0
      errors.add(:base, "tiempos mayores que 10 minutos deberían estar redondeados")
    end
  end
